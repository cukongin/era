    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="bg-green-100/50 text-green-700 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 border border-green-200">
                ...
                <?php echo e($activeYear->nama); ?>

            </div>
             <form action="<?php echo e(route('settings.year.regenerate', $activeYear->id)); ?>" method="POST"
            ...
            <!-- Backup Button -->
            <a href="<?php echo e(route('backup.store')); ?>" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2 text-sm">
    ...
    <div class="min-h-[400px]">
        ...
        <div x-show="activeTab === 'grading'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
            ...
            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 relative">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                    <div class="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg inline-flex">
                        <a href="?tab=grading&jenjang=MI" 
                           class="px-4 py-1.5 rounded-md text-sm font-bold transition-all flex items-center gap-2 <?php echo e($jenjang === 'MI' ? 'bg-white dark:bg-slate-700 shadow text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-700'); ?>">
                            MI (Cawu)
                        </a>
                        <a href="?tab=grading&jenjang=MTS" 
                           class="px-4 py-1.5 rounded-md text-sm font-bold transition-all flex items-center gap-2 <?php echo e($jenjang === 'MTS' ? 'bg-white dark:bg-slate-700 shadow text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-700'); ?>">
                            MTs (Semester)
                        </a>
                    </div>
                </div>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $periods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $periode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <form id="form-toggle-<?php echo e($periode->id); ?>" action="<?php echo e(route('settings.period.toggle', $periode->id)); ?>" method="POST" class="hidden">
                    <?php echo csrf_field(); ?>
                </form>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <?php endif; ?>

                <form action="<?php echo e(route('settings.grading.store')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div class="lg:col-span-5 space-y-6">
                            ...
                            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 mb-6">
                                ...
                                <div class="flex flex-col gap-3">
                                    <?php $__currentLoopData = $periods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $periode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="flex items-center justify-between p-3 rounded-lg border <?php echo e($periode->status == 'aktif' ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800' : 'bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700'); ?>">
                                            <div>
                                                <div class="font-bold text-sm text-slate-800 dark:text-white"><?php echo e($periode->nama_periode); ?></div>
                                                <div class="text-[10px] <?php echo e($periode->status == 'aktif' ? 'text-green-600' : 'text-slate-500'); ?>">
                                                    <?php echo e($periode->status == 'aktif' ? 'Aktif (Bisa Input)' : 'Terkunci'); ?>

                                                </div>
                                            </div>
                                            
                                            <button type="submit" form="form-toggle-<?php echo e($periode->id); ?>" 
                                                class="w-10 h-6 rounded-full transition-colors relative <?php echo e($periode->status == 'aktif' ? 'bg-green-500' : 'bg-slate-300'); ?>">
                                                <span class="absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform <?php echo e($periode->status == 'aktif' ? 'translate-x-4' : ''); ?>"></span>
                                            </button>
                                        </div>
                            ...
                            <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800 p-5">
                                <h4 class="font-bold text-slate-800 dark:text-white flex items-center gap-2 text-sm mb-4">
                                    <span class="material-symbols-outlined text-indigo-500 text-sm">tune</span> Komponen & Bobot (<?php echo e($jenjang); ?>)
                                </h4>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                                        ...
                                        <div class="relative">
                                            <input type="number" name="bobot_harian" value="<?php echo e($activeBobot->bobot_harian ?? 0); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?>
                                    ...
                                    <!-- UTS -->
                                    <div class="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-1"><?php echo e($jenjang === 'MI' ? 'Ujian Cawu (UTS)' : 'Nilai Tengah Semester (PTS)'); ?></label>
                                        <div class="relative">
                                            <input type="number" name="bobot_uts_cawu" value="<?php echo e($activeBobot->bobot_uts_cawu ?? 0); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?>
                                    ...
                                    <div class="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                                        ...
                                        <div class="relative">
                                            <input type="number" name="bobot_uas" value="<?php echo e($activeBobot->bobot_uas ?? 0); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?>
                            ...
                            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="kkm_default" value="<?php echo e($gradingSettings['kkm_default'] ?? 70); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                    ...
                                    <div>
                                        ...
                                        <select name="scale_type" <?php echo e($isLocked ? 'disabled' : ''); ?> class="w-full font-bold rounded-lg border-slate-300 focus:ring-primary text-sm">
                                            <option value="0-100" <?php echo e(($gradingSettings['scale_type'] ?? '') == '0-100' ? 'selected' : ''); ?>>0 - 100</option>
                                            <option value="1-4" <?php echo e(($gradingSettings['scale_type'] ?? '') == '1-4' ? 'selected' : ''); ?>>1 - 4</option>
                                            <option value="0-10" <?php echo e(($gradingSettings['scale_type'] ?? '') == '0-10' ? 'selected' : ''); ?>>0 - 10</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                                    <label class="flex items-center gap-3 cursor-pointer">
                                        ...
                                        <input type="checkbox" name="rounding_enable" value="1" <?php echo e(($gradingSettings['rounding_enable'] ?? 0) ? 'checked' : ''); ?> <?php echo e($isLocked ? 'disabled' : ''); ?> 
                        ...
                        <div class="lg:col-span-7 space-y-6">
                             ...
                             <div class="bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="total_effective_days" value="<?php echo e($gradingSettings['total_effective_days'] ?? 220); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                               class="w-full text-center font-bold rounded-lg border-slate-300 focus:ring-primary text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">Max Mapel Gagal</label>
                                        <input type="number" name="promotion_max_kkm_failure" value="<?php echo e($gradingSettings['promotion_max_kkm_failure'] ?? 3); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                               class="w-full text-center font-bold rounded-lg border-slate-300 focus:ring-primary text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">Min Absensi (%)</label>
                                        <input type="number" name="promotion_min_attendance" value="<?php echo e($gradingSettings['promotion_min_attendance'] ?? 85); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                               class="w-full text-center font-bold rounded-lg border-slate-300 focus:ring-primary text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">Min Sikap</label>
                                        <select name="promotion_min_attitude" <?php echo e($isLocked ? 'disabled' : ''); ?> class="w-full text-center font-bold rounded-lg border-slate-300 focus:ring-primary text-sm">
                                            <option value="A" <?php echo e(($gradingSettings['promotion_min_attitude'] ?? '') == 'A' ? 'selected' : ''); ?>>A</option>
                                            <option value="B" <?php echo e(($gradingSettings['promotion_min_attitude'] ?? '') == 'B' ? 'selected' : ''); ?>>B</option>
                                            <option value="C" <?php echo e(($gradingSettings['promotion_min_attitude'] ?? '') == 'C' ? 'selected' : ''); ?>>C</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-span-2">
                                        <label class="flex items-start gap-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-200 transition-colors">
                                            ...
                                            <input type="checkbox" name="promotion_requires_all_periods" value="1" 
                                                <?php echo e(($gradingSettings['promotion_requires_all_periods'] ?? 1) ? 'checked' : ''); ?> 
                             ...
                             <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                                <div class="px-4 py-3 bg-slate-50 border-b border-slate-200">
                                    <h4 class="font-bold text-xs uppercase text-slate-500">Interval Predikat (<?php echo e($jenjang); ?>)</h4>
                                </div>
                                <table class="w-full text-sm text-left">
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        <?php $__currentLoopData = $predicates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="px-4 py-2">
                                                <div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-700"><?php echo e($p->grade); ?></div>
                                            </td>
                                            <td class="px-4 py-2 text-center">
                                                <input type="number" name="predikat[<?php echo e($p->grade); ?>][min]" value="<?php echo e($p->min_score); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                            ...
                                            <td class="px-4 py-2 text-center">
                                                <input type="number" name="predikat[<?php echo e($p->grade); ?>][max]" value="<?php echo e($p->max_score); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                                                       class="w-16 text-center font-bold bg-transparent border-0 border-b-2 border-slate-100 focus:border-primary focus:ring-0 p-1">
                                            </td>
                                            <td class="px-4 py-2">
                                                <input type="text" name="predikat[<?php echo e($p->grade); ?>][deskripsi]" value="<?php echo e($p->deskripsi ?? ''); ?>" <?php echo e($isLocked ? 'disabled' : ''); ?> 
                             ...
                             <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mt-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="titimangsa_tempat_<?php echo e(strtolower($jenjang)); ?>" 
                                               value="<?php echo e($gradingSettings['titimangsa_tempat_' . strtolower($jenjang)] ?? ($school->kabupaten ?? '')); ?>" 
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            <input type="text" name="titimangsa_<?php echo e(strtolower($jenjang)); ?>" 
                                                value="<?php echo e($gradingSettings['titimangsa_' . strtolower($jenjang)] ?? ''); ?>" 
                                    ...
                                    <div class="col-span-1 md:col-span-2">
                                        ...
                                        <div class="relative">
                                            <input type="text" name="titimangsa_2_<?php echo e(strtolower($jenjang)); ?>" 
                                                value="<?php echo e($gradingSettings['titimangsa_2_' . strtolower($jenjang)] ?? ''); ?>" 
                            ...
                            <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl shadow-sm border border-indigo-200 dark:border-indigo-800 p-5 mt-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1">Tempat Titimangsa</label>
                                        <input type="text" name="titimangsa_transkrip_tempat_<?php echo e(strtolower($jenjang)); ?>" 
                                               value="<?php echo e($gradingSettings['titimangsa_transkrip_tempat_' . strtolower($jenjang)] ?? ($school->kabupaten ?? '')); ?>" 
                                               placeholder="Contoh: Jakarta"
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            <input type="text" name="titimangsa_transkrip_<?php echo e(strtolower($jenjang)); ?>" 
                                                value="<?php echo e($gradingSettings['titimangsa_transkrip_' . strtolower($jenjang)] ?? ''); ?>" 
                                                placeholder="Contoh: 20 Juli 2024"
                                                class="w-full font-bold rounded-lg border-indigo-300 focus:ring-indigo-500 text-sm">
                                    ...
                                    <div class="col-span-1 md:col-span-2">
                                        ...
                                        <div class="relative">
                                            <input type="text" name="titimangsa_transkrip_2_<?php echo e(strtolower($jenjang)); ?>" 
                                                value="<?php echo e($gradingSettings['titimangsa_transkrip_2_' . strtolower($jenjang)] ?? ''); ?>" 
                                                placeholder="Contoh: 17 Februari 2025 M."
                                                class="w-full font-bold rounded-lg border-indigo-300 focus:ring-indigo-500 text-sm">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="final_grade_mi" 
                                            value="<?php echo e(\App\Models\GlobalSetting::val('final_grade_mi', 6)); ?>" 
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="final_grade_mts" 
                                            value="<?php echo e(\App\Models\GlobalSetting::val('final_grade_mts', 9)); ?>" 
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                ...
                                                <input type="text" name="ijazah_range_mi" 
                                                    value="<?php echo e(\App\Models\GlobalSetting::val('ijazah_range_mi', '4,5,6')); ?>" 
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="ijazah_range_mts" 
                                                    value="<?php echo e(\App\Models\GlobalSetting::val('ijazah_range_mts', '7,8,9,1,2,3')); ?>" 
        ...
        <div x-show="activeTab === 'kkm'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
            ...
            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <form id="kkmForm" action="<?php echo e(route('settings.kkm.store')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                            ...
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-3 font-semibold text-slate-700 dark:text-white">
                                        ...
                                        <span class="text-xs font-normal text-slate-400 block"><?php echo e($mapel->kode_mapel); ?></span>
                                    </td>
                                    <td class="px-6 py-3 text-center bg-teal-50/20">
                                        <?php if($mapel->target_jenjang == 'MI' || $mapel->target_jenjang == 'SEMUA'): ?>
                                            <input type="number" name="kkm[<?php echo e($mapel->id); ?>][MI]" value="<?php echo e($kkms[$mapel->id.'-MI']->nilai_kkm ?? 70); ?>" :disabled="isLocked" class="w-20 text-center font-bold text-teal-700 rounded border-slate-200 focus:border-teal-500 focus:ring-teal-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                                        <?php else: ?>
                                            <span class="text-slate-300">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-3 text-center bg-indigo-50/20">
                                         <?php if($mapel->target_jenjang == 'MTS' || $mapel->target_jenjang == 'SEMUA'): ?>
                                            <input type="number" name="kkm[<?php echo e($mapel->id); ?>][MTS]" value="<?php echo e($kkms[$mapel->id.'-MTS']->nilai_kkm ?? 75); ?>" :disabled="isLocked" class="w-20 text-center font-bold text-indigo-700 rounded border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                                        <?php else: ?>
                                            <span class="text-slate-300">-</span>
                                        <?php endif; ?>
                                    </td>
        ...
        <div x-show="activeTab === 'identity'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <form action="<?php echo e(route('settings.identity.update')); ?>" method="POST" enctype="multipart/form-data" class="w-full">
                    <?php echo csrf_field(); ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="app_name" value="<?php echo e(\App\Models\GlobalSetting::val('app_name', 'E-Rapor')); ?>" class="w-full rounded-lg border-slate-300 dark:bg-slate-800 focus:ring-primary focus:border-primary">
                            ...
                            <div>
                                ...
                                <input type="text" name="app_tagline" value="<?php echo e(\App\Models\GlobalSetting::val('app_tagline', 'Integrated System')); ?>" class="w-full rounded-lg border-slate-300 dark:bg-slate-800 focus:ring-primary focus:border-primary">
                            ...
                            <div class="flex items-start gap-4">
                                <div class="w-32 h-32 bg-slate-100 rounded-lg border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden relative group">
                                    <?php if(\App\Models\GlobalSetting::val('app_logo')): ?>
                                        <img src="<?php echo e(asset('public/' . \App\Models\GlobalSetting::val('app_logo'))); ?>" class="w-full h-full object-contain p-2">
                                    <?php else: ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-teal-50 dark:bg-teal-900/20 p-4 rounded-xl border border-teal-100 dark:border-teal-800">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <?php $mi = \App\Models\IdentitasSekolah::where('jenjang', 'MI')->first(); ?>
                                    
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Nama Madrasah (MI)</label>
                                        <input type="text" name="nama_sekolah_mi" value="<?php echo e($mi->nama_sekolah ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500 font-bold" placeholder="MIS ...">
                                    </div>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NSM</label>
                                            <input type="text" name="nsm_mi" value="<?php echo e($mi->nsm ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500" placeholder="NSM">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NPSN</label>
                                            <input type="text" name="npsn_mi" value="<?php echo e($mi->npsn ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500" placeholder="NPSN">
                                        </div>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Alamat</label>
                                        <input type="text" name="alamat_mi" value="<?php echo e($mi->alamat ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500" placeholder="Jl. ...">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Kabupaten/Kota (Untuk Titimangsa)</label>
                                        <input type="text" name="kabupaten_mi" value="<?php echo e($mi->kabupaten ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500" placeholder="Contoh: Sidoarjo">
                                    </div>
                                    ...
                                    <div class="pt-2 border-t border-teal-200 mt-2">
                                        ...
                                        <input type="text" name="hm_name_mi" value="<?php echo e(\App\Models\GlobalSetting::val('hm_name_mi')); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500 bg-white" placeholder="Nama Lengkap & Gelar">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NIP Kepala MI</label>
                                        <input type="text" name="hm_nip_mi" value="<?php echo e(\App\Models\GlobalSetting::val('hm_nip_mi')); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-teal-500 bg-white" placeholder="NIP">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <?php $mts = \App\Models\IdentitasSekolah::where('jenjang', 'MTS')->first(); ?>

                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Nama Madrasah (MTs)</label>
                                        <input type="text" name="nama_sekolah_mts" value="<?php echo e($mts->nama_sekolah ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500 font-bold" placeholder="MTs ...">
                                    </div>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NSM</label>
                                            <input type="text" name="nsm_mts" value="<?php echo e($mts->nsm ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500" placeholder="NSM">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NPSN</label>
                                            <input type="text" name="npsn_mts" value="<?php echo e($mts->npsn ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500" placeholder="NPSN">
                                        </div>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Alamat</label>
                                        <input type="text" name="alamat_mts" value="<?php echo e($mts->alamat ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500" placeholder="Jl. ...">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">Kabupaten/Kota (Untuk Titimangsa)</label>
                                        <input type="text" name="kabupaten_mts" value="<?php echo e($mts->kabupaten ?? ''); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500" placeholder="Contoh: Sidoarjo">
                                    </div>
                                    ...
                                    <div class="pt-2 border-t border-indigo-200 mt-2">
                                        ...
                                        <input type="text" name="hm_name_mts" value="<?php echo e(\App\Models\GlobalSetting::val('hm_name_mts')); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500 bg-white" placeholder="Nama Lengkap & Gelar">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">NIP Kepala MTs</label>
                                        <input type="text" name="hm_nip_mts" value="<?php echo e(\App\Models\GlobalSetting::val('hm_nip_mts')); ?>" class="w-full text-sm rounded-lg border-slate-300 focus:ring-indigo-500 bg-white" placeholder="NIP">
                                    </div>
                                </div>
                            </div>
             ...
             <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                 ...
                 <div class="space-y-6">
                        ...
                        <form action="<?php echo e(route('settings.general.update')); ?>" method="POST">
                            ...
                            <div class="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="allow_edit_past_data" value="1" class="sr-only peer" onchange="this.form.submit()" <?php echo e(\App\Models\GlobalSetting::val('allow_edit_past_data') ? 'checked' : ''); ?>>
                    ...
                    <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                        ...
                        <form action="<?php echo e(route('settings.deadline.update')); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <div class="space-y-4">
                                <?php $__currentLoopData = $periods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $periode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3 border <?php echo e(now() > $periode->end_date ? 'border-red-200' : 'border-green-200'); ?>">
                                    <div class="flex justify-between items-center mb-2">
                                        <div>
                                            <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500"><?php echo e($periode->lingkup_jenjang); ?></span>
                                            <h4 class="font-bold text-sm text-slate-800 dark:text-white"><?php echo e($periode->nama_periode); ?></h4>
                                        </div>
                                        <div>
                                            <?php if($periode->end_date): ?>
                                                <?php if(now() > $periode->end_date): ?>
                                    ...
                                    <div class="flex gap-2 items-end">
                                        <div class="flex-1">
                                            <input type="datetime-local" name="deadlines[<?php echo e($periode->id); ?>]" 
                                                   value="<?php echo e($periode->end_date ? \Carbon\Carbon::parse($periode->end_date)->format('Y-m-d\TH:i') : ''); ?>"
                                    ...
                                    <div class="mt-2 text-right">
                                        <?php if(now() > $periode->end_date): ?>
                                             <button type="button" @click="confirmAction('<?php echo e(route('settings.deadline.toggle', ['id' => $periode->id, 'action' => 'unlock'])); ?>', 'Buka Periode Ini 24 Jam?')" class="text-[10px] font-bold text-green-600 hover:underline">Buka 24 Jam</button>
                                        <?php else: ?>
                                             <button type="button" @click="confirmAction('<?php echo e(route('settings.deadline.toggle', ['id' => $periode->id, 'action' => 'lock'])); ?>', 'Kunci Periode Ini Sekarang?')" class="text-[10px] font-bold text-red-600 hover:underline">Kunci Sekarang</button>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
                                    <?php $__empty_1 = true; $__currentLoopData = $whitelist; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td class="p-2 pl-3">
                                            <span class="font-bold block"><?php echo e($item->guru_name); ?></span>
                                            <span class="text-[10px] text-slate-500"><?php echo e($item->nama_periode); ?></span>
                                        </td>
                                        <td class="p-2 text-green-600 font-bold"><?php echo e(\Carbon\Carbon::parse($item->valid_until)->format('d M H:i')); ?></td>
                                        <td class="p-2 text-right">
                                            <form action="<?php echo e(route('settings.deadline.whitelist.remove', $item->id)); ?>" method="POST"
                                                  data-confirm-delete="true"
                 ...
                 <div class="space-y-6">
                     ...
                     <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-6">
                        ...
                        <form action="<?php echo e(route('settings.users.permissions')); ?>" method="POST" class="space-y-4">
                            <?php echo csrf_field(); ?>
                            <!-- Toggle Ekskul -->
                            <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="rapor_show_ekskul" value="1" class="sr-only peer" <?php echo e(\App\Models\GlobalSetting::val('rapor_show_ekskul', 1) ? 'checked' : ''); ?>>
                            ...
                            <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200">
                               ...
                               <label class="relative inline-flex items-center cursor-pointer">
                                   ...
                                   <input type="checkbox" name="rapor_show_prestasi" value="1" class="sr-only peer" <?php echo e(\App\Models\GlobalSetting::val('rapor_show_prestasi', 0) ? 'checked' : ''); ?>>
                                   <div class="w-9 h-5 bg-gray-200 rounded-full peer peer-checked:bg-primary peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all"></div>
                               </label>
                           </div>
                     ...
                     <div>
                         <div class="grid grid-cols-1 gap-4">
                            <a href="<?php echo e(route('settings.users.index')); ?>" class="flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-primary/50 hover:bg-primary/5 transition-all group">
                            ...
                            <a href="<?php echo e(route('settings.menus.index')); ?>" class="flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-primary/50 hover:bg-primary/5 transition-all group">
                            ...
                            <a href="<?php echo e(route('settings.pages.index')); ?>" class="flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-primary/50 hover:bg-primary/5 transition-all group">
            ...
            <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-xl border border-blue-200 dark:border-blue-800 flex flex-col justify-between">
                        ...
                        <a href="<?php echo e(route('backup.store')); ?>" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg text-center shadow-lg transition-transform hover:scale-105 flex items-center justify-center gap-2">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                        ...
                        <form action="<?php echo e(route('backup.restore')); ?>" method="POST" enctype="multipart/form-data" class="flex gap-2">
                            <?php echo csrf_field(); ?>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-[#1a2e22]">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="px-6 py-3 text-slate-500">
                                    ...
                                    <span class="text-xs text-slate-400 block"><?php echo e($backup->created_at->diffForHumans()); ?></span>
                                </td>
                                ...
                                <td class="px-6 py-3 text-right flex justify-end gap-2">
                                    <form action="<?php echo e(route('backup.restore-local', $backup->filename)); ?>" method="POST"
                                    ...
                                    <a href="<?php echo e(route('backup.download', $backup->filename)); ?>" class="text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded border border-blue-200 flex items-center gap-1" title="Download">
                                        <span class="material-symbols-outlined text-[16px]">download</span> Download
                                    </a>

                                    <form action="<?php echo e(route('backup.destroy', $backup->filename)); ?>" method="POST"
            ...
            <div class="bg-red-50 dark:bg-red-900/10 rounded-xl shadow-sm border border-red-200 dark:border-red-800 p-6 md:p-8">
                ...
                <div class="mb-8 p-6 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl shadow-xl text-white flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden max-w-6xl mx-auto">
                    ...
                    <div class="relative z-10 flex gap-3">
                        <!-- EXPORT -->
                        <form action="<?php echo e(route('settings.migration.export')); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                ...
                <dialog id="modalMigration" class="modal rounded-2xl shadow-2xl p-0 backdrop:backdrop-blur-sm">
                    <div class="w-[500px] bg-white dark:bg-slate-800 text-slate-900 dark:text-white flex flex-col">
                        ...
                        <form action="<?php echo e(route('settings.migration.import')); ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                            <?php echo csrf_field(); ?>
                ...
                <div class="mb-8 p-6 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl shadow-xl text-white flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden max-w-6xl mx-auto">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-violet-100 max-w-xl">
                            ...
                            <form action="<?php echo e(route('settings.maintenance.update-app')); ?>" method="POST" class="relative z-10" 
                  data-confirm-delete="true"
                ...
                <div class="max-w-6xl mx-auto space-y-6">
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-xl border-l-4 border-emerald-500 shadow-lg relative overflow-hidden text-slate-800 dark:text-white p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        <div class="shrink-0 w-full md:w-auto">
                            <form action="<?php echo e(route('settings.maintenance.magic-fix')); ?>" method="POST"
                                  data-confirm-delete="true"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl border border-amber-200 shadow-sm flex flex-col justify-between relative overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            <form action="<?php echo e(route('settings.maintenance.force-calcs')); ?>" method="POST"
                                  data-confirm-delete="true"
                        ...
                        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl border border-red-200 shadow-sm flex flex-col justify-between relative overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            <form action="<?php echo e(route('settings.maintenance.reset-promotion')); ?>" method="POST"
                                  data-confirm-delete="true"
                    ...
                    <div class="bg-red-50 dark:bg-red-900/10 p-6 rounded-xl border-2 border-red-500 flex flex-col items-center text-center gap-4 mt-8 col-span-full">
                        ...
                        <form action="<?php echo e(route('settings.maintenance.reset-system')); ?>" method="POST" onsubmit="return confirmReset(event)">
                            <?php echo csrf_field(); ?>
        ...
        <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div class="mt-4 flex flex-col gap-6">
                    <!-- Form New -->
                    <form action="<?php echo e(route('settings.year.store')); ?>" method="POST" class="flex gap-2">
                        <?php echo csrf_field(); ?>
                    ...
                    <div class="flex flex-col gap-2 max-h-60 overflow-y-auto">
                        ...
                        <?php $__currentLoopData = $archivedYears; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $year): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                             <span class="font-medium text-slate-700 dark:text-slate-300"><?php echo e($year->nama); ?></span>
                             <div class="flex gap-1">
                                <form action="<?php echo e(route('settings.year.toggle', $year->id)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                 ...
                                 <form action="<?php echo e(route('settings.year.destroy', $year->id)); ?>" method="POST" 
                                      data-confirm-delete="true" 
...
<dialog id="modalWhitelist" class="modal rounded-xl shadow-2xl p-0 backdrop:bg-slate-900/50 w-full max-w-md">
    <div class="bg-white dark:bg-slate-800 p-6">
        ...
        <form action="<?php echo e(route('settings.deadline.whitelist.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            ...
            <div class="mb-4">
                ...
                <select name="id_periode" class="w-full rounded-lg border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm focus:ring-primary">
                    ...
                    <option value="<?php echo e($p->id); ?>"><?php echo e($p->nama_periode); ?> (<?php echo e($p->lingkup_jenjang); ?>)</option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('settingsPage', () => ({
            ...
            isLocked: <?php echo e($isLocked ? 'true' : 'false'); ?>,
            jenjang: '<?php echo e($jenjang); ?>', // From Controller
            loading: false,
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u838039955/domains/rm.alhasany.or.id/public_html/resources/views/settings/index.blade.php ENDPATH**/ ?>