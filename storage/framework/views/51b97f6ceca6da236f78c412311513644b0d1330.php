<?php $__env->startSection('title', 'Detail Data Siswa'); ?>
...
<div class="max-w-6xl mx-auto flex flex-col gap-8 font-sans" x-data="{ isEditing: false }">
    ...
    <div class="flex items-center justify-between no-print">
        <div class="flex items-center gap-2 text-sm text-slate-500">
            <a href="<?php echo e(route('master.students.index')); ?>" class="hover:text-primary transition-colors flex items-center gap-1">
    ...
    <form action="<?php echo e(route('master.students.update', $student->id)); ?>" method="POST" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        ...
        <div class="bg-white dark:bg-[#1a2e22] rounded-xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm flex flex-col md:flex-row gap-8 relative overflow-hidden">
             ...
             <div class="flex-shrink-0 relative group">
                <div class="w-40 h-48 bg-slate-100 rounded-lg overflow-hidden border border-slate-200 shadow-inner flex items-center justify-center relative">
                    <?php if($student->foto): ?>
                        <img src="<?php echo e(asset('public/' . $student->foto)); ?>" class="w-full h-full object-cover">
                    <?php else: ?>
                        <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($student->nama_lengkap)); ?>&background=random&size=200" class="w-full h-full object-cover opacity-80">
                    <?php endif; ?>
                ...
                <div class="absolute -bottom-3 -right-3">
                     <span class="px-3 py-1 rounded-full text-xs font-bold border-2 border-white shadow-sm <?php echo e($student->status_siswa == 'aktif' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'); ?>">
                        <?php echo e(strtoupper($student->status_siswa)); ?>
             ...
             <div class="flex-1 flex flex-col justify-center">
                 <div class="flex justify-between items-start">
                     <div>
                         <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">
                            <span x-show="!isEditing"><?php echo e($student->nama_lengkap); ?></span>
                            <input x-show="isEditing" type="text" name="nama_lengkap" value="<?php echo e($student->nama_lengkap); ?>" class="text-2xl font-bold border-b-2 border-primary bg-transparent focus:outline-none w-full">
                         </h1>
                         <div class="flex items-center gap-4 text-sm text-slate-500 mb-6">
                             <div class="flex items-center gap-1">
                                 <span class="material-symbols-outlined text-[18px]">badge</span>
                                 <span>NIS: <?php echo e($student->nis_lokal ?? '-'); ?></span>
                             </div>
                             <div class="flex items-center gap-1">
                                 <span class="material-symbols-outlined text-[18px]">fingerprint</span>
                                 <span>NISN: 
                                     <span x-show="!isEditing"><?php echo e($student->nisn ?? '-'); ?></span>
                                     <input x-show="isEditing" type="text" name="nisn" value="<?php echo e($student->nisn); ?>" class="w-32 border-b border-slate-300 py-0 px-1 ml-1 text-slate-900 focus:outline-none focus:border-primary">
                                 </span>
                             </div>
                         </div>
                     </div>
                 ...
                 <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800">
                     <div>
                         ...
                         <span class="text-lg font-bold text-slate-800 dark:text-white"><?php echo e($student->kelas_saat_ini->kelas->nama_kelas ?? '-'); ?></span>
                     </div>
                     <div>
                        ...
                        <span class="text-lg font-bold text-slate-800 dark:text-white">
                            <span x-show="!isEditing"><?php echo e($student->tahun_masuk ?? date('Y')); ?></span>
                            <input x-show="isEditing" type="number" name="tahun_masuk" value="<?php echo e($student->tahun_masuk); ?>" class="w-20 bg-white border border-slate-200 rounded px-1 py-0.5 text-sm">
                        </span>
                    </div>
                     <div>
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Wali Kelas</span>
                        <span class="text-lg font-bold text-slate-800 dark:text-white text-sm truncate"><?php echo e($student->kelas_saat_ini->kelas->wali_kelas->name ?? '-'); ?></span>
                    </div>
                     <div>
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Jenjang</span>
                        <span class="text-lg font-bold text-slate-800 dark:text-white"><?php echo e(optional($student->jenjang)->kode ?? '-'); ?></span>
                    </div>
                 </div>
             </div>
        </div>
             ...
             <div class="grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-12">
                 ...
                 <div>
                     ...
                     <div x-show="!isEditing" class="text-slate-800 dark:text-white font-medium text-base">
                        <?php echo e($student->tempat_lahir); ?>, <?php echo e($student->tanggal_lahir ? \Carbon\Carbon::parse($student->tanggal_lahir)->translatedFormat('d F Y') : ''); ?>

                     </div>
                     <div x-show="isEditing" class="flex gap-2">
                        <input type="text" name="tempat_lahir" value="<?php echo e($student->tempat_lahir); ?>" placeholder="Tempat" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        <input type="date" name="tanggal_lahir" value="<?php echo e($student->tanggal_lahir); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                     </div>
                 </div>
                 <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">Jenis Kelamin</label>
                    <div x-show="!isEditing" class="text-slate-800 dark:text-white font-medium text-base">
                       <?php echo e($student->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan'); ?>

                    </div>
                    <select x-show="isEditing" name="jenis_kelamin" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        <option value="L" <?php echo e($student->jenis_kelamin == 'L' ? 'selected' : ''); ?>>Laki-laki</option>
                        <option value="P" <?php echo e($student->jenis_kelamin == 'P' ? 'selected' : ''); ?>>Perempuan</option>
                    </select>
                </div>
                 <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">NIK</label>
                    <div x-show="!isEditing" class="text-slate-800 dark:text-white font-medium text-base">
                       <?php echo e($student->nik ?? '-'); ?>

                    </div>
                    <input x-show="isEditing" type="text" name="nik" value="<?php echo e($student->nik); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div x-show="!isEditing" class="text-slate-800 dark:text-white font-medium text-base leading-relaxed">
                       <?php echo e($student->alamat_lengkap ?? '-'); ?>

                    </div>
                    <textarea x-show="isEditing" name="alamat_lengkap" rows="2" class="w-full border border-slate-300 rounded px-2 py-1 text-sm"><?php echo e($student->alamat_lengkap); ?></textarea>
                </div>
                <div>
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="bg-slate-50 dark:bg-slate-800/30 p-6 rounded-lg border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <div x-show="!isEditing" class="font-bold text-slate-800"><?php echo e($student->nama_ayah ?? '-'); ?></div>
                            <input x-show="isEditing" type="text" name="nama_ayah" value="<?php echo e($student->nama_ayah); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        </div>
                        <div>
                             <label class="text-xs text-slate-400 block mb-1">Pekerjaan</label>
                             <div x-show="!isEditing" class="font-bold text-slate-800"><?php echo e($student->pekerjaan_ayah ?? '-'); ?></div>
                             <input x-show="isEditing" type="text" name="pekerjaan_ayah" value="<?php echo e($student->pekerjaan_ayah); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50 dark:bg-slate-800/30 p-6 rounded-lg border border-slate-100 dark:border-slate-800">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="text-xs text-slate-400 block mb-1">Nama Lengkap</label>
                            <div x-show="!isEditing" class="font-bold text-slate-800"><?php echo e($student->nama_ibu ?? '-'); ?></div>
                             <input x-show="isEditing" type="text" name="nama_ibu" value="<?php echo e($student->nama_ibu); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        </div>
                         <div>
                             <label class="text-xs text-slate-400 block mb-1">Pekerjaan</label>
                             <div x-show="!isEditing" class="font-bold text-slate-800"><?php echo e($student->pekerjaan_ibu ?? '-'); ?></div>
                             <input x-show="isEditing" type="text" name="pekerjaan_ibu" value="<?php echo e($student->pekerjaan_ibu); ?>" class="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-green-500">call</span>
                         <div x-show="!isEditing" class="text-lg font-bold text-slate-800"><?php echo e($student->no_telp_ortu ?? '-'); ?></div>
                         <input x-show="isEditing" type="text" name="no_telp_ortu" value="<?php echo e($student->no_telp_ortu); ?>" class="border border-slate-300 rounded px-2 py-1 text-sm w-64">
                    </div>
                </div>
             </div>
        </div>
    ...
    <div class="bg-white dark:bg-[#1a2e22] rounded-xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm break-inside-avoid">
                ...
                <div x-data="{}">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            <?php $__empty_1 = true; $__currentLoopData = $student->riwayat_kelas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $riwayat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <tr class="bg-white dark:bg-[#1a2e22] hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors cursor-pointer group border-b border-slate-100 dark:border-slate-800">
                                ...
                                <td onclick="window.open('<?php echo e(route('reports.print', ['student' => $student->id, 'year_id' => $riwayat->kelas->id_tahun_ajaran])); ?>', '_blank')" class="px-6 py-4 font-medium text-slate-900 dark:text-white group-hover:text-primary transition-colors"><?php echo e($loop->iteration); ?></td>
                                <td onclick="window.open('<?php echo e(route('reports.print', ['student' => $student->id, 'year_id' => $riwayat->kelas->id_tahun_ajaran])); ?>', '_blank')" class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-700 dark:text-slate-200"><?php echo e($riwayat->kelas->tahun_ajaran->nama ?? '-'); ?></span>
                                        <span class="text-xs text-slate-400">Semester Ganjil &amp; Genap</span>
                                    </div>
                                </td>
                                <td onclick="window.open('<?php echo e(route('reports.print', ['student' => $student->id, 'year_id' => $riwayat->kelas->id_tahun_ajaran])); ?>', '_blank')" class="px-6 py-4 font-bold text-slate-900 dark:text-white">
                                    <?php echo e($riwayat->kelas->nama_kelas); ?>

                                    <span class="block text-xs text-slate-400 font-normal mt-0.5">Wali: <?php echo e($riwayat->kelas->wali_kelas->name ?? '-'); ?></span>
                                </td>
                                <td onclick="window.open('<?php echo e(route('reports.print', ['student' => $student->id, 'year_id' => $riwayat->kelas->id_tahun_ajaran])); ?>', '_blank')" class="px-6 py-4 text-center">
                                    <?php if(isset($gradeHistory[$riwayat->id_kelas]['periods']) && count($gradeHistory[$riwayat->id_kelas]['periods']) > 0): ?>
                                        <div class="flex flex-col gap-1 w-full max-w-[140px] mx-auto">
                                            <?php $__currentLoopData = $gradeHistory[$riwayat->id_kelas]['periods']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $period => $val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <div class="flex justify-between items-center text-xs border-b border-dashed border-slate-200 dark:border-slate-700 pb-1 last:border-0 last:pb-0">
                                                    <span class="text-slate-500"><?php echo e($period); ?></span>
                                                    <span class="font-bold <?php echo e($val < 70 ? 'text-red-500' : 'text-slate-700 dark:text-slate-300'); ?>"><?php echo e($val); ?></span>
                                                </div>
                                ...
                                <td class="px-6 py-4 text-right relative">
                                    <div class="flex items-center justify-end gap-2">
                                        <div onclick="window.open('<?php echo e(route('reports.print', ['student' => $student->id, 'year_id' => $riwayat->kelas->id_tahun_ajaran])); ?>', '_blank')" class="flex-1 text-right">
                                            <?php
            ...
            <div class="mt-8 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
                ...
                <div class="text-right">
                    ...
                    <?php if($student->status_siswa == 'lulus'): ?>
    ...
    <div class="text-center text-slate-400 text-sm mt-8 pb-8 no-print">
         &copy; <?php echo e(date('Y')); ?> Madrasah Digital System. Dokumen ini adalah salinan digital dari Buku Induk Siswa.
    </div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u838039955/domains/rm.alhasany.or.id/public_html/resources/views/master/students/show.blade.php ENDPATH**/ ?>