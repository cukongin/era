...
<?php $__env->startSection('title', 'Catatan Wali Kelas - ' . $kelas->nama_kelas); ?>
...
<div class="flex flex-col gap-6">
    ...
    <div class="mb-2">
        ...
        <form action="<?php echo e(url()->current()); ?>" method="GET" class="grid grid-cols-12 gap-3 items-center">
            <!-- Jenjang Toggle -->
            <div class="col-span-5 md:col-span-auto flex p-1 bg-slate-100 dark:bg-[#1a2332] rounded-xl border-2 border-slate-200 dark:border-slate-700 h-[46px]">
                ...
                <button type="submit" name="jenjang" value="<?php echo e($j); ?>" 
                    class="flex-1 px-3 text-sm font-bold rounded-lg transition-all flex items-center justify-center <?php echo e((request('jenjang') == $j || (empty(request('jenjang')) && $loop->first)) ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'); ?>">
                    <?php echo e($j); ?>
    ...
    <div class="grid grid-cols-1 gap-6">
        <form id="catatanForm" action="<?php echo e(route('walikelas.catatan.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <!-- Important: Pass Class ID for Admin context -->
            <input type="hidden" name="kelas_id" value="<?php echo e($kelas->id); ?>">
            
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <?php $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ak): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $note = $catatanRows[$ak->id_siswa] ?? null; // Use correct variable from controller
                    $avg = $averages[$ak->id_siswa] ?? 0;
                ?>
                <div class="bg-white dark:bg-[#1a2e22] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 hover:border-primary transition-colors student-card" data-id="<?php echo e($ak->id_siswa); ?>" data-avg="<?php echo e($avg); ?>">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                             <h3 class="font-bold text-slate-900 dark:text-white"><?php echo e($ak->siswa->nama_lengkap); ?></h3>
                             <div class="flex items-center gap-2 mt-0.5">
                                <span class="text-xs text-slate-500"><?php echo e($ak->siswa->nis_lokal); ?></span>
                                <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-mono">Avg: <?php echo e($avg); ?></span>
                             </div>
                        </div>
                    ...
                    <div class="space-y-3">
                         <div>
                            ...
                            <textarea name="catatan[<?php echo e($ak->id_siswa); ?>]" rows="3" class="w-full rounded-lg border-slate-300 dark:border-slate-700 text-sm focus:ring-primary focus:border-primary placeholder:text-slate-400 note-input" placeholder="Contoh: Tingkatkan prestasimu..."><?php echo e($note->catatan_akademik ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\erapor\resources\views/wali-kelas/catatan.blade.php ENDPATH**/ ?>