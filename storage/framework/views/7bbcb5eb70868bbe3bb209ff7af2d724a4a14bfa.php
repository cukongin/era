<?php $__env->startSection('title', 'Manajemen Data Guru'); ?>
...
<div class="flex flex-col gap-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div class="flex gap-3" x-data="{ openImport: false, openCreate: false }">
            ...
            <form action="<?php echo e(route('master.teachers.destroy-all')); ?>" method="POST"
            ...
            <div x-show="openImport" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" x-cloak>
                 <div @click.outside="openImport = false" class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-2xl w-full max-w-md p-6 border border-slate-200 dark:border-slate-800">
                    ...
                    <form action="<?php echo e(route('master.teachers.import')); ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
                        <?php echo csrf_field(); ?>
                        <div class="space-y-2">
                        ...
                        <div class="pt-2 flex gap-2">
                             <a href="<?php echo e(route('master.teachers.template')); ?>" class="flex-1 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-lg text-center text-sm hover:bg-slate-200">Download Template</a>
            ...
            <div x-show="openCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" x-cloak>
                 <div @click.outside="openCreate = false" class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-2xl w-full max-w-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar">
                        <form action="<?php echo e(route('master.teachers.store')); ?>" method="POST" id="createTeacherForm" class="space-y-6" enctype="multipart/form-data">
                            <?php echo csrf_field(); ?>
            ...
            <table class="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            <td class="px-6 py-4 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold overflow-hidden">
                                        <?php if($teacher->data_guru && $teacher->data_guru->foto): ?>
                                            <img src="<?php echo e(asset($teacher->data_guru->foto)); ?>" class="h-full w-full object-cover">
                                        <?php else: ?>
                                    ...
                                    <div>
                                        <a href="<?php echo e(route('master.teachers.show', $teacher->id)); ?>" class="font-bold hover:text-primary hover:underline transition-colors"><?php echo e($teacher->name); ?></a>
                                        <div class="text-xs text-slate-500"><?php echo e($teacher->email); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($teacher->data_guru): ?>
                                    <div class="space-y-1">
                                        <?php if($teacher->data_guru->nik): ?>
                                            <div class="px-2 py-0.5 rounded bg-slate-100 text-slate-600 border border-slate-200 text-[10px] inline-block font-mono">
                                                <?php echo e($teacher->data_guru->nik); ?>
                            ...
                            <td class="px-6 py-4">
                                <?php if($teacher->data_guru && $teacher->data_guru->mapel_ajar_text): ?>
                                    <span class="text-xs font-semibold text-slate-600 dark:text-slate-300"><?php echo e($teacher->data_guru->mapel_ajar_text); ?></span>
                                    <?php if($teacher->data_guru->pendidikan_terakhir): ?>
                                        <div class="text-[10px] text-slate-400 mt-0.5"><?php echo e($teacher->data_guru->pendidikan_terakhir); ?></div>
                                    <?php endif; ?>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4">
                                <?php echo e($teacher->data_guru->no_hp ?? '-'); ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <!-- Detail/Edit Button -->
                                    <a href="<?php echo e(route('master.teachers.show', $teacher->id)); ?>" class="p-2 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors" title="Lihat Profil / Edit">
                                        <span class="material-symbols-outlined text-[18px]">edit</span>
                                    </a>
                                    <!-- Delete Button -->
                                    <form action="<?php echo e(route('master.teachers.destroy', $teacher->id)); ?>" method="POST"
                                          data-confirm-delete="true"
...
<!-- Credential Modal -->
<?php if(session('generated_credential')): ?>
    ...
    <div class="bg-white dark:bg-[#1a2e22] rounded-xl shadow-2xl w-full max-w-md p-6 border border-slate-200 dark:border-slate-800">
        ...
        <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-100 dark:border-slate-800 space-y-3 mb-6">
            <div>
                ...
                <div class="font-medium text-slate-900 dark:text-white"><?php echo e(session('generated_credential')['name']); ?></div>
            </div>
            <div>
                <label class="text-xs font-bold text-slate-400 uppercase">Email Login</label>
                <div class="font-mono text-lg font-bold text-primary"><?php echo e(session('generated_credential')['email']); ?></div>
            </div>
            <div>
                ...
                <div class="font-mono text-lg font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-800 px-3 py-1 rounded border border-slate-200 dark:border-slate-700">
                    <?php echo e(session('generated_credential')['password']); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\XAMPP\htdocs\erapor\resources\views/master/teachers/index.blade.php ENDPATH**/ ?>