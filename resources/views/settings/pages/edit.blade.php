...
<div class="p-6 max-w-5xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Halaman</h1>
    </div>
    ...
    <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <form action="{{ route('settings.pages.update', $page->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2 space-y-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="title" value="{{ $page->title }}" class="w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-800 p-3 shadow-sm focus:ring-primary focus:border-primary" required>
                    </div>

                    <div class="space-y-1">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Link Info</label>
                        <div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg text-sm text-slate-600 dark:text-slate-400">
                            <span class="material-symbols-outlined text-lg">link</span>
                            <a href="{{ route('pages.show', $page->slug) }}" target="_blank" class="hover:underline">{{ route('pages.show', $page->slug) }}</a>
                        </div>
                    </div>

                    <div class="space-y-1">
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">Konten</label>
                        <textarea name="content" id="editor" class="w-full h-96 rounded-xl border-slate-300 dark:border-slate-700">{{ $page->content }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl space-y-4 border border-slate-200 dark:border-slate-700">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status" class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 text-sm">
                                ...
                                <option value="draft" {{ $page->status == 'draft' ? 'selected' : '' }}>Draft (Konsep)</option>
                            </select>
                        </div>
