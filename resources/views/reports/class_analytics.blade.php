        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-[#2a3441]">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-[#1f2937]/50 border-b border-slate-100 dark:border-slate-800 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            @if(isset($data['personality']) && $data['personality'] != '-')
                            <div class="text-[10px] text-slate-500 mt-1 italic">
                                "{{ \Illuminate\Support\Str::limit($data['personality'], 30) }}"
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <!-- Insight Badge (Full Width on Mobile) -->
                 @if(!empty($data['insight']))
                 <div class="mt-3 pt-3 border-t border-slate-50 dark:border-slate-800">
                     <div onclick="showAnalyticsModal('insight', 'Detail Predikat Siswa', '{{ $data['insight'] }}', 'Total Nilai: {{ number_format($data['total'], 2) }} • Absen: {{ $data['absence'] }} Hari • Sikap: {{ $data['personality'] ?? '-' }}')"
                        class="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border cursor-pointer hover:brightness-95 transition-all
